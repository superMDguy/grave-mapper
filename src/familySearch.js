...
export function getAncestorInfo() {
  return fsGet('/platform/users/current')
    .then(res =>
      fsGet(
        `/platform/tree/ancestry?person=${res.data.users[0]
          .personId}&generations=8`,
      ),
    )
    .then(res => {
      ...
      const getDetailsPromises = idLists.map(idList =>
        fsGet(`/platform/tree/persons?pids=${idList.join(',')}`),
      )
    ...
    .then(results => {
      ...
      const simplifiedPersons = persons.map(person => {
        const burialInfo = person.facts.find(fact =>
          fact.type.includes('Burial'),
        )
        ...
        const simplifiedPerson = {
          ...
          burialDate:
            burialInfo &&
            burialInfo.date &&
            burialInfo.date.normalized[0].value,
        }

        const burialPlaceDesc =
          burialInfo && burialInfo.place && burialInfo.place.description
        if (burialPlaceDesc) {
          promises.push(
            fsGet(
              `/platform/places/description/${burialInfo.place.description.slice(
                1,
              )}`,
            ).then(res => {
              const placeInfo = res.data.places[0]
              simplifiedPerson.burialPlace = {
      ...
      return Promise.all(promises).then(() => simplifiedPersons)
    })
}
